...
const updateProfileQuery = db.prepare(`UPDATE users SET pseudo = COALESCE(?, pseudo), avatar_url = COALESCE(?, avatar_url) WHERE id = ? RETURNING id, email, pseudo, avatar_url`);
